...
function compareIngredientsCB(ingredientA, ingredientB) {
    const aisleA = ingredientA.aisle;
    const aisleB = ingredientB.aisle;
    const nameA = ingredientA.name;
    const nameB = ingredientB.name;
    if (aisleA < aisleB)
        return -1;
    if (aisleA > aisleB)
        return 1;
...
function isKnownTypeCB(type) {
    ...
    return dishTypeRanking[type] === 1 || dishTypeRanking[type] === 2 || dishTypeRanking[type] === 3;
...
*/
function returnDishRankingsCB(dishA, dishB) {
    ...
    const diA = dishType(dishA)
    const diB = dishType(dishB)
    return dishTypeRanking[diA] - dishTypeRanking[diB]
}
...
function sortDishes(dishes) {
    return [...dishes].sort(returnDishRankingsCB)
...
function menuPrice(dishesArray) {
    ...
    function getAmountCB(dish) {
        return dish.pricePerServing

    }

    function SumAmountCB(dish1, dish2) {
        return dish1 + dish2
    }


    const pricePerDish = dishesArray.map(getAmountCB)
    return pricePerDish.reduce(SumAmountCB, 0)
